...
pub fn write_i16(value: i16, buf: &mut [u8], endianness: Endianness) -> BinaryWriteResult {
    if buf.len() < U16_OCTETS {
        return Err(BinaryError::new("Buf too short", BinaryErrorVariant::U16));
    }

    match write_u16(value as u16, buf, endianness) {
        Ok(n) => Ok(n),
        Err(err) => Err(err),
    }
}
...
pub fn write_i32(value: i32, buf: &mut [u8], endianness: Endianness) -> BinaryWriteResult {
    if buf.len() < U32_OCTETS {
        return Err(BinaryError::new("Buf too short", BinaryErrorVariant::U32));
    }

    match write_u32(value as u32, buf, endianness) {
        Ok(n) => Ok(n),
        Err(err) => Err(err),
    }
}
...
pub fn write_i64(value: i64, buf: &mut [u8], endianness: Endianness) -> BinaryWriteResult {
    if buf.len() < U64_OCTETS {
        return Err(BinaryError::new("Buf too short", BinaryErrorVariant::U64));
    }

    match write_u64(value as u64, buf, endianness) {
        Ok(n) => Ok(n),
        Err(err) => Err(err),
    }
}

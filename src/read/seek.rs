use crate::{
    ...
    read::{read_u16, read_u32, read_u64},
...
pub fn read_seek_u16(
    ...
    match read_u16(&data[*offset..], endianness) {
        Ok(n) => {
            *offset += U16_OCTETS;
            Ok(n)
        }
...
/// advancement of the offset would overflow the length of the `data`
/// vector the function returns an error.
pub fn read_seek_i16(
    data: &Vec<u8>,
    offset: &mut usize,
    endianness: Endianness,
) -> BinaryReadResult<i16> {
    match read_seek_u16(data, offset, endianness) {
        Ok(n) => Ok(n as i16),
        Err(err) => Err(err),
    }
}
...
pub fn read_seek_u32(
    ...
    match read_u32(&data[*offset..], endianness) {
        Ok(n) => {
            *offset += U32_OCTETS;
            Ok(n)
        }
...
/// advancement of the offset would overflow the length of the `data`
/// vector the function returns an error.
pub fn read_seek_i32(
    data: &Vec<u8>,
    offset: &mut usize,
    endianness: Endianness,
) -> BinaryReadResult<i32> {
    match read_seek_u32(data, offset, endianness) {
        Ok(n) => Ok(n as i32),
        Err(err) => Err(err),
    }
}
...
pub fn read_seek_u64(
    ...
    match read_u64(&data[*offset..], endianness) {
        Ok(n) => {
            *offset += U64_OCTETS;
            Ok(n)
        }
...
/// advancement of the offset would overflow the length of the `data`
/// vector the function returns an error.
pub fn read_seek_i64(
    data: &Vec<u8>,
    offset: &mut usize,
    endianness: Endianness,
) -> BinaryReadResult<i64> {
    match read_seek_u64(data, offset, endianness) {
        Ok(n) => Ok(n as i64),
        Err(err) => Err(err),
    }
}
